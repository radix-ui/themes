...
Toolbar has two positioning modes: **inset** (default) and **floating**.

### Inset (Default)

By default, Toolbar uses inset mode with negative margins to go edge-to-edge inside Cards, breaking out of the parent's padding.
...
<Card size="2">
  ...
  <Flex p="4">{/* Content */}</Flex>
</Card>
```

When inset (default):
- No border radius
- Edge-to-edge layout (negative margins match parent Card padding)
