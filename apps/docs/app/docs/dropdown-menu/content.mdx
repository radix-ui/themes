...
| `virtualized`     | `boolean`                   | `false`   | Enables virtualization mode. See [Virtualization](#virtualization)    |
...
function VirtualizedExample() {
  return (
    <DropdownMenu.Root>
      ...
      <DropdownMenu.Content virtualized>
        <VirtualMenu
          items={items}
...
2. Use `VirtualMenu` with your items array and a render function
3. Set a fixed `height` on VirtualMenu (required for virtualization)
4. Spread the provided `props` onto `VirtualMenu.Item`
...
| `children`          | `(item: T, props: ItemProps) => ReactNode`| —       | Render function for each item (required)         |
| `onSelect`          | `(item: T, index: number) => void`        | —       | Callback when an item is selected                |
| `estimatedItemSize` | `number \| (index: number) => number`     | `36`    | Estimated height per item. Use a function for variable heights. |
| `overscan`          | `number`                                  | `5`     | Extra items to render above/below visible area   |
| `style`             | `CSSProperties`                           | —       | Must include `height` for virtualization         |

### Render Function Props

The render function receives each item and props that **must be spread** onto your item element:
...
**Why spreading is required:**

- **`style`** — Contains `position: absolute` and `transform: translateY(...)` that positions each item at its virtual scroll position. Without it, all items stack at the top.
- **Event handlers** — `onClick`, `onMouseEnter`, `onKeyDown` enable selection, hover highlighting, and keyboard navigation.
- **ARIA attributes** — `aria-posinset` and `aria-setsize` tell screen readers "Item 5 of 1000" even though only ~20 items exist in the DOM.

| Prop              | Type              | Description                                        |
| ----------------- | ----------------- | -------------------------------------------------- |
| `isHighlighted`   | `boolean`         | Whether the item is currently highlighted          |
| `style`           | `CSSProperties`   | Positioning styles (required for virtualization)   |
| `role`            | `'menuitem'`      | ARIA role                                          |
| `tabIndex`        | `number`          | Focus management                                   |
| `aria-posinset`   | `number`          | Position in set for screen readers                 |
| `aria-setsize`    | `number`          | Total items for screen readers                     |
| `data-highlighted`| `boolean`         | CSS styling hook                                   |
| `onMouseEnter`    | `() => void`      | Hover handler                                      |
| `onClick`         | `() => void`      | Selection handler                                  |
| `onKeyDown`       | `(e) => void`     | Keyboard navigation                                |

You can destructure `isHighlighted` for custom styling while spreading the rest:

```tsx
{(item, { isHighlighted, ...props }) => (
  <VirtualMenu.Item {...props} data-active={isHighlighted}>
    {item.label}
  </VirtualMenu.Item>
)}
```
...
```tsx
const items = [
  { type: 'header', label: 'Section A' },
  { type: 'item', label: 'Item 1' },
  { type: 'item', label: 'Item 2' },
  { type: 'header', label: 'Section B' },
  // ...
];
...
>
  {(item, props) => (
    <VirtualMenu.Item {...props}>
      {item.type === 'header' ? <strong>{item.label}</strong> : item.label}
    </VirtualMenu.Item>
  )}
</VirtualMenu>
```
...
You can render custom content inside `VirtualMenu.Item`:
...
>
  {(user, props) => (
    <VirtualMenu.Item {...props}>
      <Avatar size="1" src={user.avatar} fallback={user.initials} />
      <span>{user.name}</span>
      <span style={{ color: 'var(--gray-9)' }}>{user.email}</span>
    </VirtualMenu.Item>
  )}
</VirtualMenu>
```
