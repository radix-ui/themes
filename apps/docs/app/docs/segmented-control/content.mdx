...
[View source →](https://github.com/KushagraDhawan1997/kookie-ui/blob/main/packages/kookie-ui/src/components/segmented-control.tsx)
...
Smooth spring-based indicator transitions with optimized motion curves. Separator lines automatically hide adjacent to selected items for cleaner visual hierarchy.
...
- Smooth spring-based animations for indicator transitions

### Changed

- Uses `--content-icon-size-N` tokens for consistent icon sizing across components
- Icon-only items remove horizontal padding for perfect centering
